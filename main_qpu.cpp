	...
	for (int i = 0; i < emulBufCnt; i++)
	{
		...
		for (int y = 0; y < params.height; y++)
		{
			for (int x = 0; x < params.width; x++)
			{
				...
				YUVFrameData[u*params.width + x] = ((x+params.width/(i+1))*255/params.width)%256 + ((y+params.height/(i+1))*255/params.height)%256;
				YUVFrameData[v*params.width + x] = ((x+params.width/(i+1))*255/params.width)%256 + ((y+params.height/(i+1))*255/params.height)%256;
